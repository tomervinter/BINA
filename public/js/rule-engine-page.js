...
const INSIGHT_RULES = [
  { type: 'salesPattern', title: 'זיהוי דפוס מכירות לקוח — תובנה 1',
    subrules: [
      ...
      { title: 'מגמת ירידה עקבית — ירידה משיא ואי-חזרה אליו',
        rule: 'ב-{peakDrop_windowMonths} החודשים הקלנדריים האחרונים שהסתיימו במלואם: נמצא חודש השיא במחצית הראשונה של התקופה, וממוצע המחצית השנייה נמוך ממנו ב-{peakDrop_pctThreshold}%+.', detail: 'תופס לקוח שהיה במגמת עלייה או הציג חודש חזק ואז נחלש בחדות — ולא חזר לרמה ההיא באף אחד מהחודשים שלאחר מכן. שונה מהבדיקה החדה: תופס גם כשהירידה עצמה קרתה מוקדם בתוך התקופה ומאז הרמה החדשה, הנמוכה יותר, נראית "יציבה" מול עצמה. נדרש שחודש השיא יהיה בעל מחזור של {peakDrop_minBaseRevenue}₪ לפחות. חומרה גבוהה מעל {peakDrop_highPct}%. אם אותו לקוח מקיים גם את כלל המגמה המצטברת השנתית, מופקת תובנה אחת משולבת. אם חודש השיא חופף לחג/עונה המשויכים למוצר, נבדק גם האם החג/העונה המקביל אשתקד הציג אצל הלקוח את אותו דפוס בדיוק (שיא באותו חודש חג ואז ירידה דומה באותם חודשים יחסית לאחריו) — אם כן, זהו כנראה דפוס עונתי חוזר ולא ירידה אמיתית, והתובנה לא מופקת כלל. אם קיימת חפיפה לחג/עונה אך לא ניתן לאשר דפוס חוזר (למשל, אין נתוני מכירות לשנה שעברה), התובנה כן מופקת עם ציון מפורש ותגית "לבדיקה נוספת" (ראו הגדרה כללית 8).' },
  ...
  { type: 'purchasePattern', title: 'דפוס רכישה של לקוח — תובנה 2',
    subrules: [
      ...
      { title: 'איחור ברכישה חוזרת',
        rule: 'עבור מוצר שהלקוח קונה מדי פעם (לא כל חודש): הפער מאז הרכישה האחרונה שלו חורג ב-{overdueReorder_extraMonths}+ חודשים מהפער הגדול ביותר שנצפה בהיסטוריה של הלקוח עבור אותו מוצר, ב-{overdueReorder_lookbackMonths} החודשים האחרונים.',
        detail: 'נדרשים {overdueReorder_minPurchaseMonths}+ חודשים עם רכישה בטווח הבדיקה כדי שהפער ה"רגיל" ייחשב מבוסס, ופער ממוצע של {overdueReorder_minAvgGapMonths}+ חודשים בין רכישות — כלל זה מיועד ללקוח שרוכש מדי כמה חודשים, לא ללקוח שרוכש כמעט כל חודש (עבורו הירידה כבר נתפסת בכללי המחזור/הכמות למעלה). מוצר עם תחליף מוגדר נספר יחד עם התחליף שלו כיחידה אחת. חומרה גבוהה כאשר הפער חורג ב-{overdueReorder_highExtraMonths}+ חודשים מהפער הגדול ביותר שנצפה.' },
